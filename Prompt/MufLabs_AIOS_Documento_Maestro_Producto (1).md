...
## 2.8 Modelo de negocio **[Definido contigo — validado el 2026-07-16]**

**Decisión confirmada:** BYOK total (Bring Your Own Key/Endpoint) — el usuario configura su propio proveedor de IA (Anthropic, OpenAI, Ollama, LM Studio o cualquier otro), incluyendo base URL, nombre del modelo, API key, temperatura y límite de tokens de salida. MUF Labs **no** revende ni margina inferencia. Modelo de precio: **freemium**, orientado hacia la opción de plataforma-como-servicio sobre capa de orquestación (no sobre tokens).

Esto está directamente respaldado por tu arquitectura: `04_ResourceManagement.md` (Execution Profiles configurables), `12_HardwareVirtualization.md` (independencia total de proveedor/hardware, incluyendo ejecución local) y el pilar de "Independencia de proveedor" de la Visión (Sección 1.3).

**Comparable de mercado validado:** Cline (agente de código open-source, BYOK) usa exactamente esta estructura — gratis para individuales, Teams desde $20/usuario/mes, Enterprise personalizado — y separa la tarifa de plataforma del costo de inferencia sin markup. Confirma que el modelo es viable en esta categoría, no es un experimento sin precedente.

**Propuesta de tiers [Recomendación de producto — pendiente de tu ajuste final de precios]:**

| Plan | Precio orientativo | Qué cobra | Autonomía máx. | Gobernanza |
|---|---|---|---|---|
| Individual — Gratis | $0 | Solo plataforma (BYOK obligatorio) | Nivel 0–1 | — |
| Pro | ~$12–20/mes | Multi-proyecto, memoria completa, más agentes simultáneos | Nivel 2 | — |
| Teams | ~$20–40/usuario/mes | Organization Profiles, Dashboard de Gobernanza, SSO, auditoría | Nivel 3 | Completa |
| Enterprise | Personalizado | VPC/on-prem, SLA, compliance, marketplace privado | Nivel 4 | Completa + custom |

**Implicación de producto directa:** la pantalla de Onboarding y la de configuración de proveedor deben exponer, desde el primer uso, los campos BYOK completos (proveedor, base URL, nombre del modelo, API key, temperatura, límite de tokens de salida) — esto ya está reflejado en el mockup de Organization Profiles (sección "Proveedores aprobados") y debe replicarse también a nivel de usuario individual, no solo organizacional.
