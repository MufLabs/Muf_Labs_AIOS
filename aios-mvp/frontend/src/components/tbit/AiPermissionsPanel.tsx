...
import { Shield, UserCheck, UserX, Search, RefreshCw, Filter, Plus, Trash2, Edit, Eye, Key, Database, Cpu, Network, HardDrive, AlertTriangle, CheckCircle, XCircle, Settings, RotateCcw } from "lucide-react";
        ...
        <div className="detail-panel">
          ...
          <div className="detail-content">
            ...
            <div className="detail-actions">
              {state.selectedPermission.status === "active" && (
                <button className="btn btn-secondary" onClick={() => setState((s) => ({ ...s, showEditModal: state.selectedPermission!.id }))}>
                  <Edit size={14} /> Edit
                </button>
              )}
              {state.selectedPermission.status === "active" && (
                <button className="btn btn-danger" onClick={() => revokePermission(state.selectedPermission.id)}>
