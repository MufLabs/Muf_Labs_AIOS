...
import { tbitCognitiveStore, useTBitCognitiveStore } from "../../store/useTBitCognitiveStore";

export function QuantumTelemetryRay() {
  const quantumPulseLine = useTBitCognitiveStore((state) => state.quantumPulseLine);
  const isGravityCompressing = useTBitCognitiveStore((state) => state.isGravityCompressing);
