...
import { useTBitCognitiveStore } from "../../store/useTBitCognitiveStore";
...
export function WikiLinksMesh() {
  const memoryGraph = useTBitCognitiveStore((state) => state.memoryGraph);
  const selectedMemoryNodeKey = useTBitCognitiveStore((state) => state.selectedMemoryNodeKey);
  const setSelectedMemoryNodeKey = useTBitCognitiveStore((state) => state.setSelectedMemoryNodeKey);
  const showMemoryLinks = useTBitCognitiveStore((state) => state.showMemoryLinks);
  const showMemoryAntiVits = useTBitCognitiveStore((state) => state.showMemoryAntiVits);

  const visualLinks = useMemo(() => {
    if (!memoryGraph || !showMemoryLinks) return [];
    const nodeKeys = new Set(
      memoryGraph.nodes.filter((node) => !isInternalChunkNode(node)).map((node) => node.key)
    );
  ...
  const antiFileClusters = useMemo(() => {
    ...
    return memoryGraph.nodes.filter(isMirrorableMemoryNode).map((root) => {
      ...
      const links = memoryGraph.links.filter((link) =>
        clusterKeys.has(link.sourceKey) && clusterKeys.has(link.targetKey)
      );
        ...
        <group key={`anti-file-${cluster.root.key}`}>
          ...
          {cluster.links.map((link) => (
            <Line
              ...
              points={[
                mirrorPosition(coordinatesForKey(link.sourceKey)),
                mirrorPosition(coordinatesForKey(link.targetKey)),
              ]}
              color="#ff0000"
              lineWidth={0.9}
              transparent
