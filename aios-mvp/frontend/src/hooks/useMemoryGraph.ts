...
function normalizeGraph(raw: RawGraphResponse): TBitMemoryGraph {
  const rawNodes = raw.nodes ?? [];
  const rawLinks = raw.links ?? raw.edges ?? [];

  const nodes = rawNodes.map((n, idx) => {
    const key = n.key ?? n.id ?? `node-${idx}`;
    return {
      key,
      userId: n.userId ?? n.user_id ?? DEFAULT_USER_ID,
      tags: n.tags ?? [],
      links: n.links ?? [],
      backlinks: n.backlinks ?? [],
      source: n.source ?? "tbit",
      checksum: n.checksum ?? "",
  ...
  const links = rawLinks
    .map((l) => {
      const sourceKey = l.sourceKey ?? l.source ?? "";
      const targetKey = l.targetKey ?? l.target ?? "";
      const type = l.type === "backlink" ? "backlink" : "quantum-link";
      return { sourceKey, targetKey, type };
    })
    .filter((l) => l.sourceKey && l.targetKey && nodeKeySet.has(l.sourceKey) && nodeKeySet.has(l.targetKey));

  // Build tags index from nodes if not provided
  const tags: Record<string, string[]> = raw.tags ?? {};
  if (!raw.tags) {
    for (const n of nodes) {
      for (const t of n.tags) {
        if (!tags[t]) tags[t] = [];
        if (!tags[t].includes(n.key)) tags[t].push(n.key);
      }
    }
  }
...
export function useMemoryGraph(userId: string = DEFAULT_USER_ID, autoRefresh = true) {
  ...
  const fetchGraph = useCallback(async () => {
    try {
      const raw = (await memoryCoreClient.graph(userId)) as RawGraphResponse;
