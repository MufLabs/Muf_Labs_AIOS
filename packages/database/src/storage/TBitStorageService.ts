/**
 * @aios/database - TBitStorageService
 *
 * Migrated from T-Bit (c:\Git\T-Bit\TBitStorageService.ts, 1060 lines)
 * High-level storage service providing:
 * - Serialized operation queue with file locking
 * - WAL (Write-Ahead Logging) with automatic startup recovery
 * - inject/injectMany with HMAC-signed metadata
 * - recoverData with integrity verification (zero-sum + hash)
 * - collapse (quantum annihilation) / collapseMany
 * - Snapshot/rollback support
 * - Export/import bundles with checksum verification
 * - Dynamic record quotas based on container size
 * - HMAC with timingSafeEqual to prevent timing attacks
 * - Lightweight local replica + optional remote replica
 */
...
export class TBitStorageService {
    ...
    return this.withOperation(async () => {
      ...
      try {
        if (
          ...
          Object.keys(metadata).length >= this.maxRecords()
        ) {
          throw new Error(
            `CUOTA_EXCEDIDA: limite de ${this.maxRecords()} registros activos alcanzado.`
          );
        }

        const allocation = this.findAllocatableOffsets(
          metadata,
          normalizedClave,
          primaryProjection.offsetV,
          physicalLength
        );
        walId = await this.beginWal(
          "INYECTAR",
          normalizedClave,
          length,
          physicalLength,
        ...
        if (metadata[normalizedClave]) {
          try {
            await this.container.destroyAtOffsets(
              metadata[normalizedClave].offsetV,
              metadata[normalizedClave].offsetAntiV,
              metadata[normalizedClave].length
            );
          } catch (error) {
        ...
        await this.container.writeAtOffsets(
          allocation.offsetV,
          allocation.offsetAntiV,
          normalizedDato
        );
        const assignedProjection = await this.container.calculateProjectionFromOffsets(
          allocation.offsetV,
          allocation.offsetAntiV
        );
        const metadataEntry: Omit<TBitMetadataEntry, "authTag"> = {
          ...
          authVersion: 2,
          encryption: this.container.getPayloadEncryptionStatus().enabled
            ? "AES-256-GCM"
            : "none",
    ...
    return this.withOperation(async () => {
      ...
      try {
        ...
        for (const item of prepared) {
          item.walId = await this.beginWal(
            "INYECTAR",
            item.key,
            item.length,
            item.physicalLength,
        ...
        const staleItems = prepared
          ...
          .filter(
            (entry): entry is { item: typeof prepared[number]; previous: TBitMetadataEntry } =>
              Boolean(entry.previous)
          );

        await this.container.destroyManyAtOffsets(
          staleItems.map(({ previous }) => ({
        ...
        await this.container.writeManyAtOffsets(
          prepared.map((item) => ({
        ...
        return Promise.all(
          prepared.map(async (item) => {
            const projection = await this.container.calculateProjectionFromOffsets(
              item.offsetV,
              item.offsetAntiV
            );
            return {
  ...
  async recoverData(clave: string) {
    return this.withOperation(async () => {
      ...
      const dato = await this.container.readAtOffsets(
        entry.offsetV,
        entry.offsetAntiV,
        entry.length
      );
  ...
  async collapse(clave: string) {
    return this.withOperation(async () => {
      ...
      const walId = await this.beginWal(
        "COLAPSAR",
        normalizedClave,
        entry.length,
  ...
  async collapseMany(keys: string[]): Promise<TBitBatchCollapseResult[]> {
    return this.withOperation(async () => {
      ...
      const normalizedKeys = [
        ...new Set(keys.map((key) => normalizeTBitKey(key)).filter(Boolean)),
      ];
      const metadata = await this.readMetadata();
      ...
      for (const key of normalizedKeys) {
        ...
        if (!entry) {
          results.push({
            key,
            collapsed: false,
            warning: "Clave no registrada en el indice local.",
          });
          continue;
        }
  ...
  async snapshot(label = "manual") {
    return this.withOperation(async () => {
      ...
      const safeLabel =
        label.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 40) || "manual";
      ...
      await fs.writeFile(
        ...
        JSON.stringify(
          { id, label, checksum, createdAt: new Date().toISOString() },
          null,
          2
        ),
        "utf8"
  ...
  async rollback(id: string) {
    return this.withOperation(async () => {
      ...
      const snapshotMetadata = resolve(
  ...
  async exportBundle(label = "manual") {
    return this.withOperation(async () => {
      ...
      await fs.copyFile(
        ...
        resolve(bundleDir, "universo.tbit")
      );
      await this.copyIfExistsOrWrite(
        this.config.metadataPath,
        resolve(bundleDir, "universo.tbit.meta.json"),
        "{}"
      );
      await this.copyIfExistsOrWrite(
        this.config.walPath,
        resolve(bundleDir, "universo.tbit.wal.jsonl"),
        ""
      );

      const containerHash = await this.hashFile(
        resolve(bundleDir, "universo.tbit")
      );
      const metadataHash = await this.hashFile(
        resolve(bundleDir, "universo.tbit.meta.json")
      );
      const walHash = await this.hashFile(
        resolve(bundleDir, "universo.tbit.wal.jsonl")
      );
  ...
  // ─── Private: Operation queue + file locking ───

  private async withOperation<T>(operation: () => Promise<T>): Promise<T> {
    const nextOperation = this.operationQueue.then(
    ...
    while (!lockHandle) {
      try {
        ...
        if (Date.now() - startedAt > LOCK_TIMEOUT_MS) {
          throw new Error(
            "LOCK_TIMEOUT: otro proceso mantiene bloqueado el contenedor T-BIT."
          );
        }
  ...
  private async ensureContainer(): Promise<void> {
    ...
    if (!existsSync(this.config.containerPath)) {
      await this.container.initContainer(
        this.config.containerSizeMB ?? DEFAULT_CONTAINER_SIZE_MB
      );
    }
  }
  ...
  private async startupRecovery(): Promise<void> {
    ...
    let rawWal = "";
    try {
      rawWal = await fs.writeFile(this.config.walPath, "", "utf8").then(() =>
        fs.readFile(this.config.walPath, "utf8")
      );
    } catch {
      try {
    ...
    for (const [id, groupedRecords] of recordsById) {
      ...
      if (
        ...
        try {
          await this.container.zeroFillAtOffsets(
            latest.offsetV,
            latest.offsetAntiV,
  ...
  private async readMetadata(): Promise<TBitMetadata> {
    try {
      ...
      for (const [clave, entry] of Object.entries(parsed)) {
        ...
        const projection =
          entry.offsetV === undefined || entry.offsetAntiV === undefined
        ...
        metadata[clave] = {
          ...
          encryptionKeyId:
            typeof entry.encryptionKeyId === "string" ? entry.encryptionKeyId : undefined,
  ...
  // ─── Private: WAL (Write-Ahead Logging) ───
  ...
  private signMetadata(
    clave: string,
    entry: Omit<TBitMetadataEntry, "authTag">
  ): string {
    const secret = this.config.hmacSecrets.get(this.config.hmacKeyId);
    if (!secret) {
      throw new Error(`HMAC_NO_DISPONIBLE: no existe secreto para keyId '${this.config.hmacKeyId}'.`);
    }

    const payload = JSON.stringify({ clave, ...entry });
    return createHmac("sha256", secret).update(payload, "utf8").digest("hex");
  }

  private assertMetadataSignature(clave: string, entry: TBitMetadataEntry): void {
    const { authTag, ...rest } = entry;

    if (!authTag) {
      throw new Error("METADATA_SIN_AUTENTICIDAD: registro sin authTag.");
    }

    const expected = this.signMetadata(clave, rest);
    const expectedBuffer = Buffer.from(expected, "utf8");
    const actualBuffer = Buffer.from(authTag, "utf8");

    if (
      expectedBuffer.length !== actualBuffer.length ||
      !timingSafeEqual(expectedBuffer, actualBuffer)
    ) {
      throw new Error("METADATA_FALSIFICADA: authTag HMAC no coincide.");
    }
  }

  // ─── Private: Allocation helpers ───

  private findAllocatableOffsets(
    metadata: TBitMetadata,
    clave: string,
    ...
    const allocationMap = this.buildAllocationMap(metadata);
    return this.findAllocatableOffsetsInMap(
      allocationMap,
      clave,
      primaryOffsetV,
      physicalLength
  ...
  private findAllocatableOffsetsInMap(
    ...
    for (let attempt = 0; attempt < MAX_PROBING_ATTEMPTS; attempt += 1) {
      const offsetV =
        attempt === 0
          ? primaryOffsetV
          : (primaryOffsetV + attempt * FRAME_HEADER_SIZE) %
            (this.container.getPayloadEncryptionStatus().enabled
              ? this.containerSizeBytes()
              : this.containerSizeBytes());

      const usableSize = this.usableContainerBytes();
      const offsetAntiV =
        SYSTEM_HEADER_SIZE +
        ((usableSize - (offsetV - SYSTEM_HEADER_SIZE)) % usableSize);

      const ranges = [
        ...allocationMap.circularRanges(offsetV, physicalLength),
        ...allocationMap.circularRanges(offsetAntiV, physicalLength),
      ];

      if (allocationMap.canAllocate(clave, ranges)) {
        allocationMap.allocate(clave, ranges);
        return { offsetV, offsetAntiV, probingAttempts: attempt };
      }
    }

    throw new Error(
      `COLISION_PERSISTENTE: no se encontro espacio libre despues de ${MAX_PROBING_ATTEMPTS} intentos.`
    );
  }

  private buildAllocationMap(metadata: TBitMetadata): AllocationMap {
    const lowerBound = SYSTEM_HEADER_SIZE;
    const upperBound = this.containerSizeBytes();
    const allocationMap = new AllocationMap(lowerBound, upperBound);

    const regions = Object.entries(metadata).map(([clave, entry]) => {
      const physicalLength = entry.physicalLength ?? this.container.calculateStorageLength("");

      return {
        clave,
        ranges: [
          ...allocationMap.circularRanges(entry.offsetV, physicalLength),
          ...allocationMap.circularRanges(entry.offsetAntiV, physicalLength),
        ],
      };
    });
  ...
  private async updateReplica(): Promise<void> {
    ...
    const replicaContainer = resolve(this.config.replicasDir, "universo.tbit");
    const replicaMetadata = resolve(this.config.replicasDir, "universo.tbit.meta.json");

    await fs.copyFile(this.config.containerPath, replicaContainer);

    if (existsSync(this.config.metadataPath)) {
      await fs.copyFile(this.config.metadataPath, replicaMetadata);
    } else {
      await fs.writeFile(replicaMetadata, "{}", "utf8");
    }
    ...
    if (this.config.remoteReplicaDir) {
      ...
      await fs.copyFile(
        this.config.containerPath,
        resolve(this.config.remoteReplicaDir, "universo.tbit")
      );
      await this.copyIfExistsOrWrite(
        this.config.metadataPath,
        resolve(this.config.remoteReplicaDir, "universo.tbit.meta.json"),
        "{}"
      );
    }
  }
  ...
  private async globalChecksum(): Promise<{
    ...
    const metadataHash = existsSync(this.config.metadataPath)
      ? await this.hashFile(this.config.metadataPath)
      : this.hashData("{}");
    const combinedHash = createHash("sha256")
      .update(containerHash)
      .update(metadataHash)
  ...
  private async copyIfExistsOrWrite(
    source: string,
    target: string,
    fallback: string
  ): Promise<void> {
  ...
  private validateDatoSize(dato: string): number {
    ...
    const maxBytes = this.maxDatoBytes();

    if (length > maxBytes) {
      throw new Error(
        `DATO_DEMASIADO_GRANDE: ${length} bytes excede el limite de ${maxBytes} bytes.`
      );
    }
  ...
  private maxRecords(): number {
    ...
    const configuredSizeMb = Math.max(
      1,
      Math.floor(this.containerSizeBytes() / (1024 * 1024))
    );
    return Math.min(
  ...
  private containerSizeBytes(): number {
    try {
      ...
      // If stat fails, fall back to configured size.
    }
