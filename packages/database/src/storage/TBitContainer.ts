/**
 * @aios/database - TBitContainer
 *
 * Migrated from T-Bit (c:\Git\T-Bit\TBitFileSystem.ts, 687 lines)
 * Core binary container engine for the .tbit format.
 *
 * Features:
 * - Deterministic spatial addressing via salted hash + Pi projection
 * - Vit/Anti-Vit dual-write for zero-sum integrity verification
 * - AES-256-GCM encryption-at-rest with key rotation support
 * - Circular buffer I/O with frame magic validation
 * - 3D coordinate projection from physical offsets
 */
...
const LEGACY_ENCRYPTED_PAYLOAD_OVERHEAD =
  ENCRYPTED_PAYLOAD_MAGIC.length + 1 + GCM_NONCE_SIZE + GCM_AUTH_TAG_SIZE;
const MIN_ENCRYPTED_PAYLOAD_OVERHEAD =
  ENCRYPTED_PAYLOAD_MAGIC.length + 1 + 1 + GCM_NONCE_SIZE + GCM_AUTH_TAG_SIZE;
...
export class TBitContainer {
  ...
  async writeManyAtOffsets(
    items: Array<{ offsetV: number; offsetAntiV: number; dataText: string }>
  ): Promise<void> {
    await this.ensureSizeLoaded();
    ...
    const frames = items.map((item) => {
      ...
      return { offsetV: item.offsetV, offsetAntiV: item.offsetAntiV, dataFrame, antiFrame };
    });
  ...
  async readAtOffsets(
    ...
    if (
      ...
      expectedLength !== plainBuffer.length &&
      expectedLength !== frameInfo.length
    ) {
  ...
  async destroyAtOffsets(
    ...
    if (
      expectedLength !== undefined &&
      !this.expectedLengthMatchesFrame(expectedLength, frameInfo.length)
    ) {
  ...
  async destroyManyAtOffsets(
    items: Array<{ offsetV: number; offsetAntiV: number; expectedLength?: number }>
  ): Promise<void> {
    await this.ensureSizeLoaded();
    if (items.length === 0) return;
    ...
    const frames = items.map((item) => {
      ...
      if (
        item.expectedLength !== undefined &&
        !this.expectedLengthMatchesFrame(item.expectedLength, frameInfo.length)
      ) {
        throw new Error("La longitud de destruccion no coincide con el frame fisico.");
      }
  ...
  calculateOffsets(clave: string): TBitOffsets {
    ...
    const offsetAntiV =
      SYSTEM_HEADER_SIZE +
      ((this.usableSizeInBytes() - Number(offsetBigInt)) % this.usableSizeInBytes());
  ...
  async calculateProjectionFromOffsets(
    offsetV: number,
    offsetAntiV: number
  ): Promise<TBitProjection> {
    await this.ensureSizeLoaded();
    ...
    return { offsetV, offsetAntiV, coordinates, antiCoordinates };
  }
  ...
  private readFrameInfo(offset: number): FrameInfo {
    ...
    return { length, totalLength: FRAME_HEADER_SIZE + length };
  }

  // ─── Private: Vit/Anti-Vit ───
  ...
  private encryptPayloadIfEnabled(plainBuffer: Buffer): Buffer {
    ...
    return Buffer.concat([header, authTag, ciphertext]);
  }
  ...
  private isEncryptedPayload(buffer: Buffer): boolean {
    return (
      buffer.length > LEGACY_ENCRYPTED_PAYLOAD_OVERHEAD &&
  ...
  private expectedLengthMatchesFrame(
    expectedLength: number,
    framePayloadLength: number
  ): boolean {
    return (
      expectedLength === framePayloadLength ||
  ...
  private ensureDualSegmentsDoNotOverlap(
    offsetV: number,
    offsetAntiV: number,
    length: number
  ): void {
  ...
  private circularIntervals(startOffset: number, length: number): Array<[number, number]> {
    ...
    return [
      ...
      [
        SYSTEM_HEADER_SIZE,
        SYSTEM_HEADER_SIZE + ((endOffset - SYSTEM_HEADER_SIZE) % this.usableSizeInBytes()),
      ],
    ];
  }
  ...
  private loadOrCreateSystemHeader(): void {
    ...
    try {
      ...
      if (header.subarray(0, SYSTEM_MAGIC.length).equals(SYSTEM_MAGIC)) {
        this.addressSalt = Buffer.from(
          header.subarray(SYSTEM_MAGIC.length, SYSTEM_HEADER_SIZE)
        );
        return;
      }
  ...
  private async openFileWithRetry(flags: string, purpose: string): Promise<number> {
    ...
    const code =
      (lastError as NodeJS.ErrnoException | undefined)?.code ?? "UNKNOWN";
    throw new Error(
  ...
  private normalizePhysicalOffset(offset: number): number {
    ...
    return (
      SYSTEM_HEADER_SIZE +
      ((offset - SYSTEM_HEADER_SIZE) % this.usableSizeInBytes())
    );
  }
