...
export function MemoryGraphPanel() {
  ...
  const fetchNodes = async () => {
    ...
    try {
      ...
      // Transform API response to match frontend's MemoryNode interface
      const apiNodes = response?.nodes || [];
      const nodes: MemoryNode[] = apiNodes.map((n: any) => ({
        id: n.key,
        type: (n.source === "user" ? "concept" : n.source) as MemoryNode["type"], // fallback
        label: n.key.split("::").pop() || n.key,
        content: `Tags: ${n.tags.join(", ")}\nLinks: ${n.links.join(", ")}\nSource: ${n.source}`,
        source: n.source as MemoryNode["source"],
        confidence: 0.8,
        tags: n.tags,
        links: n.links,
        backlinks: n.backlinks,
        embedding: null,
        createdAt: n.updatedAt, // fallback
        updatedAt: n.updatedAt,
        metadata: { checksum: n.checksum, userId: n.userId },
      }));
          ...
          <div className="detail-content">
            ...
            <div className="detail-actions">
              {state.selectedNode && (() => {
                ...
                return (
                  <>
                    <button className="btn btn-secondary" onClick={() => copyNodeId(node.id)}>
