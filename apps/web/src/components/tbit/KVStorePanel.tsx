...
export function KVStorePanel() {
  ...
  const createEntry = async (data: Omit<KVEntry, "key" | "createdAt" | "updatedAt" | "size" | "valuePreview" | "expiresAt">) => {
    try {
      const response = await fetch("/api/tbit/kv", {
        method: "POST",
          ...
          <div className="detail-content">
            ...
            <div className="detail-actions">
              {state.selectedEntry && (() => {
                ...
                return (
                  <>
                    <button className="btn btn-secondary" onClick={() => copyKey(entry.key)}>
                      <Copy size={14} /> Copy Key
                    </button>
                    <button className="btn btn-secondary" onClick={() => copyValue(entry.value)}>
                      <FileText size={14} /> Copy Value
                    </button>
                    <button className="btn btn-secondary" onClick={() => setState((s) => ({ ...s, showEditModal: entry.key }))}>
                      <Edit size={14} /> Edit
                    </button>
                    <button className="btn btn-danger" onClick={() => deleteEntry(entry.key)}>
...
function KVForm({ onSubmit, onCancel, initialData }: { onSubmit: (data: Omit<KVEntry, "key" | "createdAt" | "updatedAt" | "size" | "valuePreview" | "expiresAt">) => void; onCancel: () => void; initialData?: KVEntry }) {
